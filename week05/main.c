...
void init() {
  ...
  ///////////////////////////////////////
  //버튼 INIT, 버튼: PD11(S1), PD12(S2)
  GPIOD_CRH = 0x44444444; // reset
  GPIOD_CRH = 0x00088000; // 버튼 configuration
  GPIOD_IDR = 0x00000000; // 버튼 INPUT DATA REGISTER RESET
  GPIOD_BSRR = 0x00000000; // 릴레이 bsrr 리셋
 
  //릴레이 모듈: PC8 , 족보선 pc8 & 9 사용 
  GPIOC_CRH = 0x44444444; // reset
  GPIOC_CRH = 0x00000100; // 릴레이 PC8 컨피규어레이션
...
int main(void)
{
  ...
  while(1){
    if (~GPIOD_IDR & 0x00000800) { // pd 11번 버튼 눌렸을때
    ...
    } else if (~GPIOD_IDR & 0x00001000) { //pd 12번 버튼 눌렸을 때
      //     GPIOC_BSRR = 0x0100; // pc8 set
